...
class GeneradorImagenesUI:
    ...
    def setup_ui(self):
        ...
        tk.Label(header, text="🎨 GENERADOR DE IMÁGENES CON IA", 
                 font=('Segoe UI', 18, 'bold'), bg=self.colors['bg_main'], fg=self.colors['accent']).pack(side=tk.LEFT)
        
        tk.Button(header, text="🔙 VOLVER", bg=self.colors['bg_panel'], fg="white",
                 font=('Segoe UI', 10, 'bold'), command=self.volver_al_dashboard,
    ...
    def setup_panel_izquierdo(self, parent):
        ...
        estilo_frame = tk.LabelFrame(parent, text="🎨 CONFIGURACIÓN DE ESTILO", 
                                    font=('Segoe UI', 12, 'bold'), bg=self.colors['bg_panel'], 
                                    fg=self.colors['accent'], relief="raised", borderwidth=2)
        estilo_frame.pack(fill=tk.X, pady=(0, 15))
        
        tk.Label(estilo_frame, text="Estilo:", bg=self.colors['bg_panel'], fg="white",
                 font=('Segoe UI', 10)).pack(anchor="w", padx=15, pady=(10, 5))
        
        estilos = ["Modern", "Vibrant", "Minimalist", "Professional", "Creative", "Elegant"]
        self.estilo_combo = ttk.Combobox(estilo_frame, textvariable=self.estilo_seleccionado, 
                                        values=estilos, font=('Segoe UI', 10), state="readonly")
        self.estilo_combo.pack(fill=tk.X, padx=15, pady=(0, 10))
        
        tk.Label(estilo_frame, text="Tamaño:", bg=self.colors['bg_panel'], fg="white",
                 font=('Segoe UI', 10)).pack(anchor="w", padx=15, pady=(10, 5))
        
        tamaños = ["1080x1080", "1920x1080", "1080x1920", "1200x630", "800x800"]
        self.tamano_combo = ttk.Combobox(estilo_frame, textvariable=self.tamano_seleccionado, 
                                         values=tamaños, font=('Segoe UI', 10), state="readonly")
        self.tamano_combo.pack(fill=tk.X, padx=15, pady=(0, 10))
    ...
    def on_producto_seleccionado(self, event=None):
        ...
        if seleccion in self.productos_data:
            ...
            if self.promocion_seleccionado:
                self.generar_prompt()
    ...
    def on_promocion_seleccionada(self, event=None):
        ...
        if seleccion in self.promociones_data:
            self.promocion_seleccionado = self.promociones_data[seleccion]
    ...
    def generar_prompt(self):
        ...
        if not self.promocion_seleccionado:
            messagebox.showwarning("Aviso", "Por favor selecciona una promoción")
            return
            
        # Extraer tipo de promoción
        promo_texto = self.promocion_seleccionado['texto']
        if "CUPÓN:" in promo_texto:
            tipo_promo = "CUPÓN"
            descuento = promo_texto.split("-")[1].strip()
    ...
    def crear_prompt_personalizado(self, producto, tipo_promo, descuento, estilo, tamano):
        ...
        prompts_base = {
            "CUPÓN": f"""Crea una imagen profesional de cupón de descuento para {nombre_producto}.
...
Idioma: Todo el texto debe estar en español""",
            ...
            "COMBO": f"""Diseña una imagen atractiva de promoción combo con {nombre_producto}.
Producto: {nombre_producto}
Precio: ${precio}
Descuento: {descuento}
...
Idioma: Todo el texto debe estar en español""",
            ...
            "MAYORISTA": f"""Genera un gráfico de oferta mayorista para {nombre_producto}.
Producto: {nombre_producto}
Precio: ${precio}
Descuento: {descuento}
...
Idioma: Todo el texto debe estar en español""",
            ...
            "GENERAL": f"""Crea una imagen publicitaria profesional para {nombre_producto}.
Producto: {nombre_producto}
Precio: ${precio}
...
def ejecutar_generador_imagenes(empresa_id, nombre_negocio):
    ...
    try:
        ...
        def on_closing():
            try:
                # Llamar al método de volver al dashboard que maneja todo correctamente
                app.volver_al_dashboard()
            except:
                root.destroy()
