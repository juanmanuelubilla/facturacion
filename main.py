...
class NexusLauncher:
    ...
    def obtener_config_db(self, empresa_id):
        try:
            ...
            with conn.cursor() as cursor:
                cursor.execute("SELECT nombre FROM empresas WHERE id=%s LIMIT 1", (empresa_id,))
                res = cursor.fetchone()
                nombre = res['nombre'] if isinstance(res, dict) else res[0]
    ...
    def cargar_lista_empresas(self):
        try:
            ...
            with conn.cursor() as cursor:
                ...
                self.lista_items = [f"{e['id']} - {e['nombre']}" if isinstance(e, dict) else f"{e[0]} - {e[1]}" for e in empresas]
    ...
    def create_widgets(self):
        ...
        rol = (self.usuario_actual['rol'] if isinstance(self.usuario_actual, dict) else self.usuario_actual[2]).lower()
        nombre_u = (self.usuario_actual['nombre'] if isinstance(self.usuario_actual, dict) else self.usuario_actual[1]).upper()
        id_u = self.usuario_actual['id'] if isinstance(self.usuario_actual, dict) else self.usuario_actual[0]
        ...
        # --- FILA 1: NIVEL ADMINISTRATIVO (Sólo Admin) ---
        if rol == 'admin':
            ...
            self.crear_modulo(row_admin, "EMPRESAS", "Gestión de Locales.", self.colors['settings'], "gestor_empresas_ui.py", "🏢", extra_arg=args_comunes)
            self.crear_modulo(row_admin, "PERSONAL", "Gestión de usuarios.", self.colors['users'], "usuarios_ui.py", "👥", extra_arg=args_comunes)

        # --- FILA 2: NIVEL GERENCIAL (Jefe y Admin) ---
        if rol in ['jefe', 'admin']:
            ...
            self.crear_modulo(row_jefe, "INVENTARIO", "Stock y precios.", self.colors['accent'], "gestion_ui.py", "📦", extra_arg=args_comunes)
            self.crear_modulo(row_jefe, "FINANZAS", "Caja y movimientos.", self.colors['finanzas'], "finanzas.py", "💰", extra_arg=args_comunes)
            self.crear_modulo(row_jefe, "REPORTES", "Gráficos estadísticos.", self.colors['reportes'], "reportes.py", "📈", extra_arg=args_comunes)
            self.crear_modulo(row_jefe, "PROMOS", "Cupones y Descuentos.", self.colors['promo'], "promociones_ui.py", "🎟️", extra_arg=args_comunes)

        # --- FILA 3: NIVEL OPERATIVO (Todos) ---
        ...
        self.crear_modulo(row_operativo, "VENTAS", "Punto de Venta POS.", self.colors['success'], "app_gui.py", "🛒", extra_arg=args_comunes)

        # Botón cerrar sesión
        tk.Button(self.root, text="⬅ CERRAR SESIÓN", font=('Segoe UI', 8, 'bold'), bg=self.colors['bg'], fg="#666", 
                  relief="flat", command=self.mostrar_login, cursor="hand2").place(x=20, y=20)
    ...
    def crear_modulo(self, master, titulo, desc, color, script, icono, extra_arg=None):
        ...
        tk.Label(card, text=desc, font=('Segoe UI', 8), bg=self.colors['card'], fg=self.colors['text_dim'], wraplength=130, justify="center").pack(pady=10)
        
        tk.Button(card, text="INGRESAR", font=('Segoe UI', 8, 'bold'), bg=color, fg="white", relief="flat", padx=25, pady=10, 
                  command=lambda: self.lanzar_script(script, extra_arg), cursor="hand2").pack()
